...
## Table of Contents
 ...
 - [SuperNOVAS vs. astropy](#supernovas-vs-astropy)
...
<a name="supernovas-vs-astropy"></a>
## SuperNOVAS vs. astropy

__astropy__ is widely used and is known for its simplicity and elegance, despite its sluggishness (see the 
[benchmarks](#benchmarks)). And, while C is fast, it also has a bad reputation for being tedious and ugly. However, 
below is a side-by-side comparison of calculating CIRS apparent positions in __astropy__ vs. __SuperNOVAS__ for 
Antares for a given date and observer location:
...
novas_make_frame(NOVAS_FULL_ACCURACY, 
  &loc, &time, POLAR_DX, POLAR_DY, 
  &frame);
<br>
// apparent coordinates
sky_pos apparent;
novas_sky_pos(&source, &frame, 
  NOVAS_CIRS, &apparent);
</code></pre>
</td>
</tr>
...
Yes, perhaps the C code is a little more verbose, but not unbearably so. Overall, __astropy__ isn't a whole lot 
simpler or easier than __SuperNOVAS__, expcept perhaps that __SuperNOVAS__ currently requires that you set the leap 
seconds and Earth orientation parameters (`DUT1`, `POLAR_DX`, `POLAR_DY`) explictly, while __astropy__ will download 
these automatically (as long as you are online -- otherwise it will also require you to specify these just as 
__SuperNOVAS__ does).
