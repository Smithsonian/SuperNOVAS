...
static void breakdown(double value, int decimals, int *h, int *m, int *s, long long *ss) {
  long long mult = (long long) pow(10.0, decimals);
  long long u;

  *ss = (long long) floor(value * 86400L * mult + 0.5);
...
/**
 * Prints a time in hours as hh:mm:ss[.S...] into the specified buffer, with
 ...
 * @param decimals    Requested number of decimal places to print for the seconds.
...
int novas_print_hms(double hours, enum novas_separator_type sep, int decimals, char *restrict buf, int len) {
  ...
  if(len < 1)
    return novas_error(-1, EINVAL, fn, "invalid buffer len: %d", len);
...
/**
 * Prints an angle in degrees as [-]ddd:mm:ss[.S...] into the specified buffer, with the
 * specified precision. The degrees component is always printed as 4 characters (up to 3 digits
