...
### Added
 ...
 - `cio_array()` can now parse the original ASCII CIO locator data file (`data/CIO_RA.TXT`) efficiently, no longer 
   requiring a platform-specific binary translation via the `cio_file` tool.
...
### Changed
 ...
 - `cio_file` tool parses interval from header rather than the less precise differencingo of first two record
   timestamps. This leads to `cio_array()` being more precisely centered on matching date entries, e.g. J2000.
