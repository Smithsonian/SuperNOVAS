...
int main() {
  ...
  for(int i = 0; i < NOVAS_PLANETS; i++) {
    ...
    if(!test.equals(std::to_string(i) + "novas_id()", p.novas_id(), i)) n++;
    if(!test.equals(std::to_string(i) + "naif_id()", p.naif_id(), (int) novas_to_naif_planet(p.novas_id()))) n++;
    if(!test.equals(std::to_string(i) + "de_number()", p.de_number(), (int) novas_to_dexxx_planet(p.novas_id()))) n++;
  }
  ...
  for(int i = 0; i < NOVAS_PLANETS; i++) {
     ...
     else if(!test.equals("for_name(" + names[i] + ").novas_id()", opt.value().novas_id(), i)) n++;
  }
