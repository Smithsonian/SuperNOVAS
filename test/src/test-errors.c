...
static int test_cio_array() {
  ...
  if(check("cio_array:bin:empty", 1, cio_array(2341952.0, 5, x))) n++;
  ...
  if(check("cio_array:ascii:no-data", 1, cio_array(2341952.0, 2, x))) n++;

  set_cio_locator_file("bad-cio-data/bad-4.txt");
  if(check("cio_array:ascii:corrupt:first", -1, cio_array(2341952.0, 2, x))) n++;

  set_cio_locator_file("bad-cio-data/bad-5.txt");
  if(check("cio_array:ascii:corrupt", 6, cio_array(2341952.0, 2, x))) n++;
